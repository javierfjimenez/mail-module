...
$(function () {
    ...
    if (emailUserList.find("li").length) {
        emailUserList.find("li").on("click", function (e) {
            ...
            // console.log(uid);
            $.get(
                window.location.origin + "/api/get/mail/" + uid,
                (data, status) => {
                    console.log(data.data);
                    // let user = data.find(user => user.id == $(this).data('id'));
                    emailDetails.find("div.mail-message").html(data.data.body);
                    emailDetails
                        .find("h5.personal-from")
                        .text(data.data.from.personal);
                    emailDetails
                        .find("span.email-from")
                        .text(data.data.from.mail);
                    emailDetails
                        .find("td.email-from")
                        .text(data.data.from.mail);
                    emailDetails.find("td.email-to").text(data.data.to);
                    emailDetails.find("td.email-date").text(data.data.to);
                    emailDetails
                        .find("span.attachment-count")
                        .text(
                            parseInt(data.data.attachments.count) == 1
                                ? data.data.attachments.count + " Adjunto"
                                : data.data.attachments.count + " Adjuntos"
                        );
                    console.log(
                        data.data.attachments.count +
                            (parseInt(data.data.attachments.count) <= 1)
                            ? " Adjzunto"
                            : " Adjuntos"
                    );
                    // emailDetails.find('.email-user-info .user-avatar').attr('src', assetPath + 'images/avatars/' + user.avatar);
                    // emailDetails.find('.email-user-info .user-avatar').attr('alt', user.name);
                }
            );
